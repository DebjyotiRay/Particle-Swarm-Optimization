...
class PSOAgent:
    ...
    def update_velocity(self, swarm):
        ...
        # self.velocity = swarm.inertia*v_t + swarm.cognitive*np.random.uniform(0, 2, 1)*cog_dist + swarm.social*np.random.uniform(0, 2, 1)*soc_dist
        r1, r2 = np.random.uniform(0, 2, 1), np.random.uniform(0, 2, 1)
        cog_vel = c1 * r1 * (self.P_best_pos[0] - self.P_best_pos[0])
        social_vel = c2 * r2 * (global_best_position[i] - self.pos[i])
        self.vel[i] = swarm.inertia * v_t + cog_vel + social_vel
